...
#define kWordTangentAuto		((EtByte *)"auto")
...
wordAsTangentType (EtByte *type)
{
	...
	if (engineUtilStringsMatch (type, kWordTangentAuto)) {
		return (kTangentAuto);
	}
	return (kTangentSmooth);
}
...
assembleAnimCurve (EtReadKey *firstKey, EtInt numKeys, EtBoolean isWeighted, EtBoolean useOldSmooth)
{
	...
	while (nextKey != kEngineNULL) {
        ...
        if (key->inTangentType == kTangentPlateau || key->inTangentType == kTangentAuto )
        {
            ...
            {         
                ...
                else
                {
					...
					*/
	                ...
	                {
	                    if( key->inTangentType == kTangentPlateau )
	                    {
	                        key->inTangentType = kTangentFlat;
	                    }
	                    else /*(key->inTangentType == kTangentAuto)*/
	                    {
							inTanY = -py*3.0;
        ...
        if (key->outTangentType == kTangentPlateau || key->outTangentType == kTangentAuto )
        {
            ...
            {         
                ...
                else
                {
					...
					*/
	                ...
	                {
	                    if( key->outTangentType == kTangentPlateau )
	                    {
	                        key->outTangentType = kTangentFlat;
	                    }
	                    else /*(key->outTangentType == kTangentAuto)*/
	                    {
							outTanY = ny*3.0;
